...
app.post('/sms', (req, res) => {
    ...
    request(obj, function (error, response, body) {
        ...
        $('.job-position').each(function (i, elem) {
            job_position[i] = $(this).text();
        });
        $('.job-company').each(function (i, elem) {
            job_company[i] = $(this).text();
        });
        $('.job-location').each(function (i, elem) {
        ...
        let ur = obj.uri + '?q=' + args[0] + '&radius=20w=' + args[1] + '&promotedonly=' + args[2];
        mess += `\nVisit: ${ur}`;
